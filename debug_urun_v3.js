...
async function debugSelectUrun() {
    ...
    // Changing namespace to the one used in variations
  ...
  <soap:Body>
    <SelectUrun xmlns="http://tempuri.org/">
        ...
        <s>
            ...
            <KayitSayisi xmlns="http://schemas.datacontract.org/2004/07/">50</KayitSayisi>
        </s>
    </SelectUrun>
  </soap:Body>
</soap:Envelope>`;
    ...
    fs.writeFileSync('debug_products_v3.xml', text, 'utf8');
    
    const count = (text.match(/<a:UrunKarti>/g) || []).length;
    console.log(`Found ${count} <a:UrunKarti> tags using base datacontract namespace.`);
}
